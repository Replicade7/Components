...
local function UpdateArrow(plr, isNPC)
    ...
    local char = isNPC and plr or plr.Character
    if char and char:FindFirstChild("HumanoidRootPart") and char:FindFirstChild("Humanoid") and char.Humanoid.Health > 0 then
        local rootPart = char.HumanoidRootPart
        local _, onScreen = CurrentCamera:WorldToViewportPoint(rootPart.Position)
        if not onScreen then
            if not isNPC and Environment.Settings.HideTeam and plr.Team == LocalPlayer.Team then
                arrowFill.Visible = false
                arrowOutline.Visible = false
                return
            end
            ...
            if rel.Magnitude > 0.1 then
                ...
                if not isNPC and Environment.Settings.TeamColorEnabled and plr.TeamColor then
    ...
    pcall(function()
        for _, plr in pairs(Players:GetPlayers()) do
            if plr ~= LocalPlayer then
                UpdateArrow(plr, false)
            end
        end
        if Environment.Settings.WorkForNpc then
            for npc, _ in pairs(Environment.NPCObjects) do
                UpdateArrow(npc, true)
            end
        end
    end)
end)
...
local function CreateVisuals(player, isNPC)
    ...
    return {Box = {box, boxOutline, fillBox}, HealthBar = {healthBar, healthOutline}, Tracer = {tracer, tracerOutline1, tracerOutline2}, Name = name, Distance = distance, Player = player, IsNPC = isNPC, Box3D = box3d, CornerBox = cornerBox}
end
local function UpdateVisuals(entry)
    ...
    if not isNPC and Environment.Settings.HideTeam and player.Team == LocalPlayer.Team then
    ...
    if Environment.Box.ViewBox == "3D" then
        ...
        if onScreen and boxEnabled and espEnabled then
            ...
            if Environment.Box.FillEnabled then
                ...
                box3d.box_quad6.PointC = Vector2new(Top4.X, Top4.Y)
                box3d.box_quad6.PointD = Vector2new(Bottom4.X, Bottom4.Y)
            end
            for i = 1, 12 do
                box3d["box_line" .. i].Visible = true
                box3d["box_line" .. i].Color = isNPC and Environment.Box.Color or CoreFunctions.GetTeamColor(player)
            ...
            for i = 1, 6 do
                ...
                box3d["box_quad" .. i].Color = isNPC and Environment.Box.Color or CoreFunctions.GetTeamColor(player)
    ...
    elseif Environment.Box.ViewBox == "Corner" then
        for _, obj in pairs(visuals.Box) do obj.Visible = false end
        for _, obj in pairs(visuals.Box3D) do obj.Visible = false end
        local color = isNPC and Environment.Box.Color or CoreFunctions.GetTeamColor(player)
        ...
        box.Size = size
        box.Color = isNPC and Environment.Box.Color or CoreFunctions.GetTeamColor(player)
        ...
        if Environment.Box.FillEnabled then
            ...
            fillBox.Size = size
            fillBox.Color = isNPC and Environment.Box.Color or CoreFunctions.GetTeamColor(player)
    ...
    if Environment.Tracer.Enabled then
        ...
        tracer.From = fromPos
        tracer.To = toPos
        tracer.Color = isNPC and Environment.Tracer.Color or CoreFunctions.GetTracerColor(player)
    ...
    if Environment.Pname.Enabled then
        ...
        name.Text = isNPC and player.Name or player.Name
        name.Position = Vector2new(position.X + size.X / 2, position.Y - 15)
        name.Color = (not isNPC and Environment.Settings.TeamColorEnabled and player.Team and player.Team.TeamColor.Color) or Environment.Pname.Color
    else
        name.Visible = false
    end
    if Environment.Pdistance.Enabled then
        ...
        distance_obj.Color = (not isNPC and Environment.Settings.TeamColorEnabled and player.Team and player.Team.TeamColor.Color) or Environment.Pdistance.Color
    else
...
function SkeletonESP:getPlayerColor(plr, isNPC)
    return (not isNPC and Environment.Settings.TeamColorEnabled and plr.TeamColor and plr.TeamColor.Color) or Environment.Skeleton.Color
end
function SkeletonESP:updateComponents(components, character, plr, isNPC)
    if not isNPC and Environment.Settings.HideTeam and plr.Team == LocalPlayer.Team then
        ...
        if Environment.Skeleton.Enabled and Environment.Settings.Enabled then
            ...
            if head then
                ...
                if headPos.Z > 0 then
                    ...
                    components.HeadCircle.Color = self:getPlayerColor(plr, isNPC)
                    ...
                    for _, connection in ipairs(connections) do
                        ...
                        if partA and partB then
                            local line = components.SkeletonLines[connection[1].."-"..connection[2]] or self:createDrawing("Line", {
                                Thickness = Environment.Skeleton.Thickness,
                                Color = self:getPlayerColor(plr, isNPC)
                            })
                            ...
                            line.Visible = true
                            line.Color = self:getPlayerColor(plr, isNPC)
...
local function SkeletonESPFunction(plr, isNPC)
    local function Updater()
        ...
        local function UpdateESP()
            ...
            renderConnection = RunService.RenderStepped:Connect(function()
                pcall(function()
                    if (isNPC and not plr.Parent) or (not isNPC and not Players:FindFirstChild(plr.Name)) then
                        renderConnection:Disconnect()
                    ...
                    local character = isNPC and plr or plr.Character
                    if character and character:FindFirstChild("Humanoid") and character:FindFirstChild("HumanoidRootPart") and character.Humanoid.Health > 0 and character:FindFirstChild("Head") then
                        ...
                        if OnScreen then
                            ...
                            skeletonInstance:updateComponents(skeletonInstance.espCache[plr], character, plr, isNPC)
                        else
                            skeletonInstance:hideComponents(skeletonInstance.espCache[plr] or {})
                        end
                    else
...
local function IsNPC(model)
    return model:FindFirstChild("Humanoid") and model:FindFirstChild("HumanoidRootPart") and not Players:GetPlayerFromCharacter(model)
end
local function InitESP()
    for _, player in ipairs(GetPlayers()) do
        if player ~= LocalPlayer then
            local entry = {Visuals = CreateVisuals(player, false), Player = player, IsNPC = false}
            ...
            SkeletonESPFunction(player, false)
        end
    end
    Players.PlayerAdded:Connect(function(newPlayer)
        if newPlayer == LocalPlayer then return end
        local entry = {Visuals = CreateVisuals(newPlayer, false), Player = newPlayer, IsNPC = false}
        ...
        SkeletonESPFunction(newPlayer, false)
    end)
    Players.PlayerRemoving:Connect(function(leftPlayer)
        ...
        skeletonInstance:removeEsp(leftPlayer)
    end)
end
local function InitNPCESP()
    for _, model in ipairs(Workspace:GetChildren()) do
        if IsNPC(model) then
            local entry = {Visuals = CreateVisuals(model, true), Player = model, IsNPC = true}
            Environment.NPCObjects[model] = entry
            local connection
            connection = RunService.RenderStepped:Connect(function()
                if not Environment.NPCObjects[model] then
                    connection:Disconnect()
                    return
                end
                UpdateVisuals(entry)
            end)
            SkeletonESPFunction(model, true)
        end
    end
    local npcAdded
    npcAdded = Workspace.ChildAdded:Connect(function(child)
        if IsNPC(child) then
            local entry = {Visuals = CreateVisuals(child, true), Player = child, IsNPC = true}
            Environment.NPCObjects[child] = entry
            local connection
            connection = RunService.RenderStepped:Connect(function()
                if not Environment.NPCObjects[child] then
                    connection:Disconnect()
                    return
                end
                UpdateVisuals(entry)
            end)
            SkeletonESPFunction(child, true)
        end
    end)
    local npcRemoved
    npcRemoved = Workspace.ChildRemoved:Connect(function(child)
        if Environment.NPCObjects[child] then
            local entry = Environment.NPCObjects[child]
            for _, obj in pairs(entry.Visuals.Box) do obj:Remove() end
            for _, obj in pairs(entry.Visuals.HealthBar) do obj:Remove() end
            for _, obj in pairs(entry.Visuals.Tracer) do obj:Remove() end
            for _, obj in pairs(entry.Visuals.Box3D) do obj:Remove() end
            ...
            entry.Visuals.Name:Remove()
            entry.Visuals.Distance:Remove()
            Environment.NPCObjects[child] = nil
        end
        skeletonInstance:removeEsp(child)
    end)
end
InitESP()
RunService.RenderStepped:Connect(function()
    if not Environment.Settings.Enabled or not Environment.Chams.Enabled then
        ...
        for npc, _ in pairs(Environment.NPCObjects) do
            if npc:FindFirstChild("PlayerHighlight") then
                npc.PlayerHighlight:Destroy()
            end
        end
        return
    end
...
_G_WorksforNPCesp = function(state)
    Environment.Settings.WorkForNpc = state
    if state then
        InitNPCESP()
    else
        for npc, entry in pairs(Environment.NPCObjects) do
            for _, obj in pairs(entry.Visuals.Box) do obj:Remove() end
            for _, obj in pairs(entry.Visuals.HealthBar) do obj:Remove() end
            for _, obj in pairs(entry.Visuals.Tracer) do obj:Remove() end
            for _, obj in pairs(entry.Visuals.Box3D) do obj:Remove() end
